...
class CeasarCipher(ICipher):
    ...
    def encrypt(self) -> str:
        ...
        for char in self.text:
            char_code = ord(char)
            encrypted_char_code = self.__get_encrypted_char_code(char_code, shift)
    ...
    def __get_encrypted_char_code(cls, char_code: int, shift: int) -> int:
        if char_code in [cls.SPACE_CODE]:
            ...
            if char_code in cls.__get_letters_minus_shift(
                shift, capital=True
            ) + cls.__get_letters_minus_shift(shift, capital=False):
                encrypted_char_code = char_code + shift
            else:
                if char_code not in cls.__get_letters_minus_shift(shift, capital=True):
                    encrypted_char_code = char_code + cls.__get_reverse_shift(
                        shift, capital=True
                    )
                elif char_code not in cls.__get_letters_minus_shift(
                    shift, capital=False
                ):
                    encrypted_char_code = char_code + cls.__get_reverse_shift(
                        shift, capital=False
